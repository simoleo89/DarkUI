...
DarkUI is the Flash-inspired dark experience running on the modern Nitro V3
client and the PixiJS 8 renderer. The application keeps the original DarkUI
sprites, typography, window chrome, toolbar proportions, and interaction
language while inheriting the current features, configuration system, tests,
and runtime behavior from `DEV/ui`.

## Renderer resolution

The development and production builds use the first renderer found in this
order:

1. `NITRO_RENDERER_ROOT`
2. `../Nitro_Render_V3`
3. `../renderer`
4. `../../DEV/renderer`

Game assets follow `NITRO_FILES_ROOT`, then the sibling `../Nitro-Files`, and
finally `../../DEV/Nitro-Files`. This keeps a regular standalone installation
portable while allowing this workspace to consume the maintained `DEV`
renderer and assets directly.
