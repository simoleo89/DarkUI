...
describe('Toolbar feature config', () => {
    it('keeps optional custom toolbar buttons behind global config flags', () => {
        ...
        expect(source.match(/buildersClubEnabled\s*&&/g)).toHaveLength(3);
        expect(source.match(/fortuneWheelEnabled\s*&&/g)).toHaveLength(3);
        expect(source).toContain("darkAction(localizeWithFallback('toolbar.icon.label.buildersclub'");
        expect(source).toContain("darkAction(localizeWithFallback('toolbar.icon.label.fortunewheel'");
