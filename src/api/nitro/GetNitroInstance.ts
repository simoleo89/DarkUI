...
// Nitro_Render_V3 (2.1.0) removed the monolithic `Nitro` singleton in favour of
// standalone Get*() accessors. This client still calls GetNitroInstance().<member>
// in many places, so we keep that facade and delegate every member to the new
// accessor. Getters stay lazy so each access resolves the live manager (which only
// exists after the App bootstrap has run).
const instance: any = {
