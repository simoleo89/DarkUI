...
export class FriendlyTime {
    ...
    public static format(seconds: number, key: string = '', threshold: number = 3): string {
        ...
        if (seconds > threshold * FriendlyTime.MINUTE)
            return FriendlyTime.getLocalization('friendlytime.minutes' + key, Math.round(seconds / FriendlyTime.MINUTE));
    ...
    public static shortFormat(seconds: number, key: string = '', threshold: number = 3): string {
        if (seconds > threshold * FriendlyTime.YEAR)
            return FriendlyTime.getLocalization('friendlytime.years.short' + key, Math.round(seconds / FriendlyTime.YEAR));
        ...
        if (seconds > threshold * FriendlyTime.DAY)
            return FriendlyTime.getLocalization('friendlytime.days.short' + key, Math.round(seconds / FriendlyTime.DAY));

        if (seconds > threshold * FriendlyTime.HOUR)
            return FriendlyTime.getLocalization('friendlytime.hours.short' + key, Math.round(seconds / FriendlyTime.HOUR));
